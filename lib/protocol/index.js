...
module.exports = function Protocol(stream, config, zeronet) {
  ...
  function debugStream(stream) {
    if ((process.env.NODE_DEBUG || "").indexOf("zeronet-packets") != -1) {
      stream.on("data", data => console.log("raw--in", config.isServer, data.toString()))
      const ow = stream.write.bind(stream)
      stream.write = function (data) {
        console.log("raw-out", config.isServer, (data ? data : "<no-data>").toString())
        ow(data)
      }
    }
  }
  ...
  */
  ...
  self.tlsUpgrade = function (options, cb, asy) {
    ...
    //stream.write(Buffer.from([0, 0, 0, 0]))
