...
function ModuleDocs(mod, basepath) {
  ...
  self.render = parts => {
    let r = rend()
    r.add("### API for " + mod)
    parts.map(p => p.render()).forEach(p => r.add(p, true))
    r.add("Autogenerated by [a script](/docs-gen/lib/index.js?raw=true)", true)
    r.add("")
    return r
  }
}
...
function ClassDocs(name, jsdoc, part) {
  ...
  self.render = () => {
    ...
    if (jsdoc.undocumented) {
      ...
      r.add("#### Constructor - " + renderFncArgs("new " + part.display.split("/").pop().replace(/-/g, "_") + "." + self.name, jsdoc))
      r.add(renderDesc(jsdoc), true)
      r.add(renderParamDesc(jsdoc), true)
    }
