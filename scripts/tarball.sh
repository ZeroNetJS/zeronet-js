...
for file in $(dir -w 1 | grep "^zeronet-"); do
  if [ -d $file ]; then
    cd $file
    GZIP=-n tar cvfz ../$file.tar.gz --sort=name --mtime='2015-10-21 00:00Z' --owner=0 --group=0 --numeric-owner $ex . | sed "s|^|$file: |g" || GZIP=-n tar cvfz ../$file.tar.gz --mtime='2015-10-21 00:00Z' --owner=0 --group=0 --numeric-owner $ex . | sed "s|^|$file: |g"
    ex_re
    cd ..
  fi
done
